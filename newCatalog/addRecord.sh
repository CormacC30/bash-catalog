...
if [ -a "$catalog_file" ]; then
	...
	echo "Creating new catalogue"
    	touch "$catalog_file"
	echo "\"Number\",\"Track Name\",\"Artist\",\"Album\",\"Genre\",\"Duration\"" >> "$catalog_file" #!!!
fi
...
if [ ${#trackName} -gt 20 ]; then
	trackName=`./foldInput.sh "$trackName"`
fi
read -p "Enter Artist: " artist
if [ ${#artist} -gt 20 ]; then
        artist=`./foldInput.sh "$artist"`
fi
read -p "Enter Album: " album
if [ ${#album} -gt 20 ]; then
        album=`./foldInput.sh "$album"`
fi
read -p "Enter Genre: " genre
if [ ${#genre} -gt 20 ]; then
        genre=`./foldInput.sh "$genre"`
fi
validate_input() {
	local input="$1"
...
echo
head -n 1 musictracks.csv | sed 's/","/ /g' #print the column headers using sed  #!!!!!
echo										#!!! Add in your printf statement here
echo
echo "Your full Catalogue:"
echo
echo
./splitColumns.sh musictracks.csv
echo
echo
awk 'BEGIN {FS=","}{gsub(/"/, "", $0);
if (NR == 1) 	{
